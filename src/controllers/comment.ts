...
export default class Comment {
    ...
    async editComment(commentId: string, newComment: string, userId: string): Promise<any> {
        const comment = await this.#comments.findById(commentId);
        if (!comment) return null;

        // Check if user owns this comment
        if (comment.user.toString() !== userId.toString()) return "unauthorized";

        const isCommentEmpty = !newComment || newComment.trim().length === 0;
        if (isCommentEmpty) return 204;
