...
export default class Books {
    ...
    async GetBooks(page: number = 1, limit: number = 5) {
        ...
        try {
            const books = await this.#books.find().populate("user", "name").sort({ _id: -1 }).skip(skip).limit(limit).exec();
            return { books };
        } catch (error) {
    ...
    async GetBook(id: string) {
        ...
        const book = await this.#books
            ...
            .sort({ _id: -1 })
            .populate("user", "name username pp")
            .exec();
        return book;
    }
    ...
    async GetBooksFromPublisher(page: number = 1, limit: number = 5): Promise<bookType[]> {
        const skip = (page - 1) * limit;
        try {
            const books = await this.#books.find({ tag: "Publish" }).populate("user", "name username pp").sort({ _id: -1 }).skip(skip).limit(limit).exec();
            return books || [];
        } catch (error) {
    ...
    async GetBooksFromQuestion(page: number = 1, limit: number = 5): Promise<bookType[]> {
        const skip = (page - 1) * limit;
        try {
            const books = await this.#books.find({ tag: "Question" }).populate("user", "name username pp").sort({ _id: -1 }).skip(skip).limit(limit).exec();
            return books || [];
        } catch (error) {
    ...
    async GetBooksBySearch(page: number = 1, limit: number = 5, tag: string, term: string): Promise<bookType[]> {
        ...
        try {
            const books = await this.#books
                .find({
                    $and: [{ title: { $regex: term, $options: "i" } }, { tag: tag }],
                })
                .sort({ _id: -1 })
                .skip(skip)
                .limit(limit)
